    ...
    try:
        ...
        for i in range(1, len(entries)):
            ...
            if current.blockIndex != previous.blockIndex + 1:
                return {
                    "valid": False,
                    "error": f"Block index sequence broken between {previous.blockIndex} and {current.blockIndex}.",
            ...
            if current.prevHash != previous.blockHash:
                return {
                    "valid": False,
                    "error": f"Ledger link broken. Block {current.blockIndex} previous-hash reference mismatch.",
                    "compromisedBlockIndex": current.blockIndex
                }

            re_calculated = calculate_block_hash(
                current.blockIndex,
                current.voucherNo,
                current.amount,
                current.debitAcc,
            ...
            if current.blockHash != re_calculated:
                return {
                    "valid": False,
                    "error": f"Integrity check failed. Block {current.blockIndex} content was modified.",
                    "compromisedBlockIndex": current.blockIndex
                }

        return {"valid": True}
    except Exception as e:
        return {"valid": False, "error": f"Ledger query exception: {str(e)}"}
