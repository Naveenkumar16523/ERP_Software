...
export default function SignIn() {
  const { setToken, setCurrentUser, setDemoMode, addToast, theme } = useERPStore();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // First attempt real backend login
      ...
      const userObj = {
        ...data.user,
        name: data.user.name || `${data.user.firstName || 'User'} ${data.user.lastName || ''}`.trim()
      };
      
      setCurrentUser(userObj);
      setDemoMode(false);
      
      addToast(`Welcome back, ${userObj.firstName || userObj.name || 'User'}!`, 'success');
    } catch (err) {
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative border border-white/10 focus-within:border-indigo-500 rounded-lg bg-black/20 overflow-hidden transition-all duration-200">
                ...
                <input 
                  ...
                  className="w-full bg-transparent border-0 px-9 py-2.5 text-white placeholder:text-slate-600 text-sm outline-none" 
                  placeholder={authView === 'login' ? "admin@example.com" : "Your email address"}
                  id="email"
                />
              </div>
            </div>
