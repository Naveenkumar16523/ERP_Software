...
async function main() {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      const containsAppJsx = contentStr.includes('App.jsx');
      const containsFollowCursor = contentStr.includes('FollowCursor');
      const containsRainbow = contentStr.includes('RainbowButton');
      
      if (containsAppJsx && (containsFollowCursor || containsRainbow)) {
